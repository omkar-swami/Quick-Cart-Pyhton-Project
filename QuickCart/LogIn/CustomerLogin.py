...
import pymysql
CustLogin = Blueprint('CustLogin', __name__)


def connection():
    return pymysql.connect(host='localhost', user='root', password='', database='quickcart')

@CustLogin.route("/CustLogin", methods=['GET', 'POST'])
def CustLogin_index():
    custlist=[]
    error = None
    if request.method == 'POST':
        ...
        conn = connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM customer where Cust_Email=%s and Cust_password=%s",(email,password ) )
        custlist = cursor.fetchall()
        if len(custlist)==1:
            session["Cust_id"] = custlist[0][0]
            session['Cust_Name'] = custlist[0][1]
  ...
  <div class="login-card">
    ...
    <form action="/CustLogin" method="POST">
      ...
      <button type="submit" class="btn btn-primary w-100">Login</button>
        <p id="new-user">New user <a href="/CustRegistration">Create Account </a></p>
      {% if error %}
